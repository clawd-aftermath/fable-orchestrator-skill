...
For routine reversible code changes—normal edits recoverable through git, with no deploy or schema/data migration and less than ~30 minutes invested before the approach can be falsified—implement the change, run the relevant checks, and make the working-tree result durable; then call a fresh rich plain advisor before committing. Send the actual diff/code excerpts, changed-file scope, test or command output, evidence for and against the implementation, and remaining assumptions. Address any revise or stop verdict before creating the commit.

Call advisor before implementation only when requirements are genuinely ambiguous, an architectural or novel-API choice is load-bearing, the proposed operation may be destructive or irreversible, or the next approach is likely to consume more than ~30 minutes before it can be falsified. Consult mid-task when an assumption breaks, evidence conflicts with the plan, tests fail unexpectedly, scope grows materially, you are stuck, or you are considering a change of approach.
